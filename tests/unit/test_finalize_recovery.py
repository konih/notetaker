...
def _container_with(conn) -> Container:
    return Container(
...
def test_finds_ended_session_with_all_unknown_transcript(tmp_path) -> None:
...
def test_skips_session_with_a_real_speaker_label(tmp_path) -> None:
    conn = open_connection(f"sqlite:////{tmp_path}/db.sqlite3")
    c = _container_with(conn)
    session = _make_session(c.sessions, ended_at=datetime(2026, 1, 1, 10, 0, 0))
    _add_segment(c.transcripts, session, "unknown")
...
def test_skips_session_still_recording(tmp_path) -> None:
    conn = open_connection(f"sqlite:////{tmp_path}/db.sqlite3")
    c = _container_with(conn)
...
def test_skips_session_with_no_transcript(tmp_path) -> None:
    conn = open_connection(f"sqlite:////{tmp_path}/db.sqlite3")
    c = _container_with(conn)
...
def test_ended_after_bounds_the_recovery_window(tmp_path) -> None:
    conn = open_connection(f"sqlite:////{tmp_path}/db.sqlite3")
    c = _container_with(conn)
