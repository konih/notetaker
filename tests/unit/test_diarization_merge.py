...
def test_overlap_seconds_disjoint() -> None:
    ...
    assert overlap_seconds(t0, t0 + timedelta(seconds=1), t0 + timedelta(seconds=2), t0 + timedelta(seconds=3)) == 0.0


def test_overlap_seconds_partial() -> None:
    t0 = datetime(2026, 1, 1, 12, 0, 0)
    assert overlap_seconds(t0, t0 + timedelta(seconds=10), t0 + timedelta(seconds=5), t0 + timedelta(seconds=15)) == 5.0
...
def test_pick_speaker_largest_overlap() -> None:
    ...
    assert pick_speaker_by_overlap(t0 + timedelta(seconds=2), t0 + timedelta(seconds=8), diar) == "speaker_2"
...
def test_pick_speaker_unknown_when_no_overlap() -> None:
    ...
    assert pick_speaker_by_overlap(t0 + timedelta(seconds=5), t0 + timedelta(seconds=6), diar) == "unknown"
